...
var viewModel = function (data) {
    var self = this;
    self[window.cateObj.msgCategory] = ko.observableArray(data);//eg:weibo、weixin
    ...
    //        }
    //    };
    //    window.baseTools['edit'+msgCategory](postData, function (data) {
    //        self[msgCategory].replace(t, newObject);
    //    });
    //};
    //初始化应急处理弹出层
...
$(function () {
    ...
    $(".left_arr").click(function () {
        ...
        if(window.msgCategory=='weixin'){otherWith=670;}
...
function getParameters() {
    ...
    if (isAdvancedSearch) {//高级搜索
        ...
        if (dimValue != "") {
            parameters.query[window.cateObj.parame.t2] = dimValue;
...
function searchObjs(params) {
    window.baseTools['get'+msgCategoryFir](params, function (result) {//getWeibo
        $("#totalcount").text(result.totalcount);
        options.dataSource(result.rows);
        initPagination(options, result.totalcount);
        //initSwitch();
